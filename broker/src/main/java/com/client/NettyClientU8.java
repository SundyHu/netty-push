...
public class NettyClientU8 {
	static Logger logger = Logger.getLogger(NettyClientU8.class);
...
//    private static final EventExecutorGroup group = new DefaultEventExecutorGroup(20);//根据通道相应的事件对通道进行处理，而这个处理也有可能是多个线程的多线程处理
    ...
    public NettyClientU8(int port, String host){
...
//              socketChannel.pipeline().addLast(new ObjectDecoder(1024,ClassResolvers.cacheDisabled(this.getClass().getClassLoader())));
                ...
                socketChannel.pipeline().addLast(new NettyClientHandlerU8(NettyClientU8.this));
            }
        });
    }
...
//	        });
    ...
    public static void init() throws IOException{
		InputStream is = NettyClientU8.class.getResourceAsStream("/DBConfig.properties");
    ...
    public static void main(String[]args) throws Exception{
    	...
    	Constants.setClientId(Config.apps.get("U8clientID"));
        NettyClientU8 clientCRM=new NettyClientU8(Integer.parseInt(Config.apps.get("port")), Config.apps.get("host"));
    	try {
			clientCRM.start();
			clientCRM.doConnect();
