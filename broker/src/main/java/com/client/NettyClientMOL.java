...
public class NettyClientMOL {
	static Logger logger = Logger.getLogger(NettyClientMOL.class);
...
//    private static final EventExecutorGroup group = new DefaultEventExecutorGroup(20);//根据通道相应的事件对通道进行处理，而这个处理也有可能是多个线程的多线程处理
    ...
    public NettyClientMOL(int port, String host){
...
//              socketChannel.pipeline().addLast(new ObjectDecoder(1024,ClassResolvers.cacheDisabled(this.getClass().getClassLoader())));
                ...
                socketChannel.pipeline().addLast(new NettyClientHandlerMOL(NettyClientMOL.this));
            }
        });
    }
...
//	        });
    ...
    public static void init() throws IOException{
		InputStream is = NettyClientMOL.class.getResourceAsStream("/DBConfig.properties");
    ...
    public static void main(String[]args) throws Exception{
    	...
    	Constants.setClientId(Config.apps.get("MOLclientID"));
        NettyClientMOL clientCRM=new NettyClientMOL(Integer.parseInt(Config.apps.get("port")), Config.apps.get("host"));
    	try {
			clientCRM.start();
			clientCRM.doConnect();
